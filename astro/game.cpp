        ...
        if(choice%3==0){//生成地面障碍物与金币
            ...
            if(extra%1==0){
                ...
                if(rand()%3)img.load(QString(bigcoin));
                    ...
                    i->restore=true;
                    i->effect.setSource(QUrl::fromLocalFile("../sound/extend.wav"));
                    i->effect.setVolume(0.25);
                }
                (*i).setImage(img);
                (*i).extra=true;
                coin1.push_back(i);
...
void game::keyPressEvent(QKeyEvent *event)
{
    if(gameover&&event->key()==Qt::Key_R){
        ...
        if(p) p->keyPress(event);
    }
}
void game::keyReleaseEvent(QKeyEvent *event)
{
    if(p) p->keyRelease(event);
}
...
void game::gameisover(){
    ...
    static QSoundEffect over;
    over.setSource(QUrl::fromLocalFile("../sound/defeat.wav"));
    over.setVolume(1);
    over.play();
    hurtImgAlpha=0;
